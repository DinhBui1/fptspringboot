...
public interface SanPhamRepository extends JpaRepository<SanPham,Integer> {
    ...
    @Query("SELECT u FROM SanPham u WHERE u.isDeteted=0 and u.danhMuc.maDM= ?1")
    List<SanPham> getSanPhamByMaDMNotDelete(int MaDM);

    @Query("SELECT u FROM SanPham u ORDER BY u.maSP ASC LIMIT 8")
    List<SanPham> get8SanPham();

    @Query("SELECT u FROM SanPham u where u.isDeteted = 0 ORDER BY u.maSP ASC LIMIT 8")
    List<SanPham> get8SanPhamNotDelete();
